...
Finds the <item> whose <enclosure url=...> contains "WarmKitty-<version>.zip" and
...
def find_item(root, version):
    needle = "WarmKitty-%s.zip" % version
...
def inject(appcast_path, version, lang_files):
    ...
    if item is None:
        sys.exit("No <item> with enclosure for WarmKitty-%s.zip in %s"
